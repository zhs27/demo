...
def train_model(modelQ,modelQh, train_loader,val_loader,cfg):
    ...
    def train_one_epoch(modelQ,modelQh):
        bar=tqdm(train_loader,ncols=100,unit='batch',leave=False)
        epsum=run_one_epoch(modelQ,modelQh,bar,'train',loss_func=loss_func,optimizerQ=optimizerQ,optimizerQh=optimizerQh)
        summary={"loss/train":np.mean(epsum['loss'])}
        return summary
        
        
    def eval_one_epoch(modelQ,modelQh):
        bar=tqdm(val_loader,ncols=100,unit='batch',leave=False)
        epsum=run_one_epoch(modelQ,modelQh,bar,"valid",loss_func=loss_func)
...
def run_one_epoch(modelQ,modelQh,bar,mode,loss_func,optimizerQ=None,optimizerQh=None,show_interval=10, upfreq = 10):
    ...
    device=next(modelQh.parameters()).device
    ...
    for i, (x_cpu,y_cpu) in enumerate(bar):
        ...
        if mode=='train':
            ...
            #Train model Q_hat#
            optimizerQh.zero_grad()
            pred,loss=modelQh(x,modelQ)
            loss.backward()
            optimizerQh.step()

            #Update model Q in every N batches# 
            if i % upfreq == 0:
                modelQ.load_state_dict(modelQh.state_dict())   
